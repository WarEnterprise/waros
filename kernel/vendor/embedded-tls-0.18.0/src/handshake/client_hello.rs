    ...
    CipherSuite: TlsCipherSuite,
{
        ...
        Provider: CryptoProvider,
    {
        ...
        let mut legacy_session_id = [0; 32];
        provider.rng().fill_bytes(&mut legacy_session_id);
    ...
    pub(crate) fn encode(&self, buf: &mut CryptoBuffer<'_>) -> Result<(), TlsError> {
        ...
        // Use TLS 1.3 compatibility mode with a non-empty legacy session ID.
        // Some public servers and middleboxes reject an empty session ID even
        // though RFC 8446 permits it.
