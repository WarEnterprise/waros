...
| WarShield Pass 4 operational resilience | INTEGRATED | `kernel/src/pkg/update.rs`; `kernel/src/main.rs`; shell `warpkg` / `recovery` commands | Real offline local signed-bundle stage/apply path plus persisted boot-health and recovery state; still no secure boot, remote control plane, or A/B slot model |
...
| Package manager + signature path | INTEGRATED | `kernel/src/pkg/*`; `crates/waros-pkg/*`; `kernel/src/pkg/smoke.rs` | Packages install only after signed-bundle verification plus payload-digest checks; Pass 4 adds explicit local stage/apply, but repository metadata, rotation, revocation, and remote orchestration remain deferred |
