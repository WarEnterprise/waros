...
| WarShield Pass 1 hardening | INTEGRATED | `kernel/src/security/*`; `kernel/src/main.rs`; `kernel/src/net/tcp.rs` | Current scope is audit hooks, outbound TCP firewall decisions, stack ASLR on the WarExec load path, W^X loader enforcement, and selected capability gates |
